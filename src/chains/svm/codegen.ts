  ...
  for (const [name, acc] of pdas) {
    ...
    for (const seed of seeds) {
      if (seed.kind === 'const' && seed.value) {
        ...
        seedExprs.push(`(typeof ${paramName} === 'string' ? Buffer.from(${paramName}) : typeof ${paramName} === 'number' ? Buffer.from(new Uint8Array(new BigUint64Array([BigInt(${paramName})]).buffer)) : ${paramName})`)
      }
    }
...
function collectUsedDefinedTypes(idl: AnchorIdl): string[] {
  const white = new Set<string>()  // not yet visited
  ...
  function visitDefined(name: string) {
    ...
    if (typeDef) {
      ...
      const fields = typeDef.type.kind === 'struct'
        ? (typeDef.type.fields ?? [])
...
function mapBorshCodec(t: AnchorType, idl: AnchorIdl, property?: string): string {
  ...
  if (typeof t === 'string') {
    switch (t) {
      case 'bool': return `borsh.bool(${prop})`
      case 'u8': return `borsh.u8(${prop})`
      case 'u16': return `borsh.u16(${prop})`
      case 'u32': return `borsh.u32(${prop})`
      case 'u64': return `borsh.u64(${prop})`
      case 'u128': return `borsh.u128(${prop})`
      case 'u256': return `borsh.u256(${prop})`
      case 'i8': return `borsh.i8(${prop})`
      case 'i16': return `borsh.i16(${prop})`
      case 'i32': return `borsh.i32(${prop})`
      case 'i64': return `borsh.i64(${prop})`
      case 'i128': return `borsh.i128(${prop})`
      case 'i256': return `borsh.i256(${prop})`
      case 'f32': return `borsh.f32(${prop})`
      case 'f64': return `borsh.f64(${prop})`
      case 'string': return `borsh.str(${prop})`
      case 'pubkey': case 'publicKey': return `borsh.publicKey(${prop})`
      case 'bytes': return `borsh.vecU8(${prop})`
      default: return `borsh.u8(${prop}) /* unknown: ${t} */`
    }
  }
...
function generateAccounts(idl: AnchorIdl): string {
  ...
  for (const acc of idl.accounts ?? []) {
    ...
    if (typeDef) {
      lines.push(`  return ${camelCase(typeName)}AccountLayout.decode(accountInfo.data.subarray(8)) as types.${typeName}`)
    } else {
...
function mapBorshCodecForAccounts(t: AnchorType, idl: AnchorIdl, property?: string): string {
  ...
  if ('vec' in t) return `borsh.vec(${mapBorshCodecForAccounts(t.vec, idl)}${prop ? `, ${prop}` : ''})`
  if ('option' in t) return `borsh.option(${mapBorshCodecForAccounts(t.option, idl)}${prop ? `, ${prop}` : ''})`
  if ('coption' in t) return `borsh.coption(${mapBorshCodecForAccounts(t.coption, idl)}${prop ? `, ${prop}` : ''})`
  if ('array' in t) return `borsh.array(${mapBorshCodecForAccounts(t.array[0], idl)}, ${t.array[1]}${prop ? `, ${prop}` : ''})`
  if ('defined' in t) {
